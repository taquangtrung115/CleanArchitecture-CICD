...
export default function RolePage() {
    ...
    return (
        <MainCard title="Role Management">
            ...
            <Typography variant="h6" sx={{ mb: 2 }}>Thêm Role mới</Typography>
            <form onSubmit={handleSubmit} style={{ marginBottom: 24 }}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                        <TextField label="Description" name="description" value={form.description} onChange={handleChange} fullWidth disabled={loading} />
                    </Grid>
                    <Grid item xs={12} sm={6} md={4}>
                        <TextField label="Role Code" name="roleCode" value={form.roleCode} onChange={handleChange} fullWidth required disabled={loading} />
                    </Grid>
                    <Grid item xs={12}>
                        <Button type="submit" variant="contained" disabled={loading}>Tạo Role</Button>
                    </Grid>
                </Grid>
                {error && <Typography color="error" sx={{ mt: 1 }}>{error}</Typography>}
            </form>
            <Typography variant="h6" sx={{ mb: 1 }}>Danh sách Role</Typography>
                ...
                <Table size="small">
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow><TableCell colSpan={3}>Không có role nào</TableCell></TableRow>
                        ) : roles.map((r) => (
