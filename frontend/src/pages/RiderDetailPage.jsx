...
export default function RiderDetailPage() {
    ...
    return (
        <div style={{ maxWidth: 600, margin: '40px auto', padding: 24, border: '1px solid #ccc', borderRadius: 8 }}>
            ...
            {edit ? (
                <>
                    <input name="firstName" value={form.firstName} onChange={e => setForm(f => ({ ...f, firstName: e.target.value }))} placeholder="First Name" />
                    <input name="lastName" value={form.lastName} onChange={e => setForm(f => ({ ...f, lastName: e.target.value }))} placeholder="Last Name" />
                    <input name="nickname" value={form.nickname} onChange={e => setForm(f => ({ ...f, nickname: e.target.value }))} placeholder="Nickname" />
                    <input name="height" value={form.height} onChange={e => setForm(f => ({ ...f, height: e.target.value }))} placeholder="Height" type="number" />
                    <input name="weight" value={form.weight} onChange={e => setForm(f => ({ ...f, weight: e.target.value }))} placeholder="Weight" type="number" />
                    <button onClick={handleSave} disabled={loading}>{loading ? 'Saving...' : 'Save'}</button>
                    <button onClick={() => setEdit(false)}>Cancel</button>
                </>
            ) : (
                <>
                    <div><b>Name:</b> {rider.fullName}</div>
                    <div><b>Racing Number:</b> {rider.racingNumber}</div>
                    <div><b>Country:</b> {rider.countryFlag} {rider.countryName}</div>
                    <div><b>Height:</b> {rider.height} cm</div>
                    <div><b>Weight:</b> {rider.weight} kg</div>
                    <div><b>Status:</b> {rider.isActive ? 'Active' : 'Retired'}</div>
                    <div><b>Team:</b> {rider.currentTeamName || 'Free Agent'}
                        {rider.currentTeamId && (
                            <RemoveRiderButton riderId={id} onRemoved={() => window.location.reload()} />
                        )}
                    </div>
            ...
            <button onClick={handleRetire} disabled={loading}>Retire</button>
            <button onClick={handleComeback} disabled={loading}>Comeback</button>
            <hr />
            <button onClick={() => {
                if (window.confirm('Bạn có chắc chắn muốn xóa rider này không?')) handleDelete();
            }} style={{ color: 'red' }} disabled={loading}>Delete Rider</button>
        </div>
    );
}
