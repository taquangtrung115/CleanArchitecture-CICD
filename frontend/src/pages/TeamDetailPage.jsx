...
export default function TeamDetailPage() {
  ...
  const handleAddRider = async () => {
    setLoading(true);
    const res = await transferRider(
      addRider.riderId,
      { teamId: id, seasonId: addRider.seasonId, joinDate: addRider.joinDate },
  ...
  return (
    <div style={{ maxWidth: 700, margin: '40px auto', padding: 24, border: '1px solid #ccc', borderRadius: 8 }}>
      ...
      {edit ? (
        <>
          ...
          <input
            ...
            onChange={(e) => setForm((f) => ({ ...f, countryFlag: e.target.value }))}
        ...
        <>
          ...
          <div>
            <b>Short Name:</b> {team.shortName}
          </div>
          <div>
            <b>Country:</b> {team.countryFlag} {team.countryName}
          </div>
          <div>
            <b>Founded:</b> {team.foundedYear?.slice(0, 10)}
          </div>
          <div>
      ...
      <input
        ...
        value={addRider.seasonId}
        onChange={(e) => setAddRider((a) => ({ ...a, seasonId: e.target.value }))}
      />
      <input type="date" value={addRider.joinDate} onChange={(e) => setAddRider((a) => ({ ...a, joinDate: e.target.value }))} />
      <button onClick={handleAddRider} disabled={loading}>
        Add Rider
      </button>
      <hr />
      <button
